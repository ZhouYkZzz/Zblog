...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      {
        message: "未配置 API_KEY，无法调用通义千问 API。请先在 .env 中填写 API_KEY。"
      },
      { status: 400 }
    );
  }
  ...
  const response = await fetch(`${baseUrl}/responses`, {
    ...
    body: JSON.stringify({
      model,
      input: `你是计算机研究生的论文阅读助手。请只输出 JSON，不要输出 Markdown。字段必须是 researchQuestion, method, result, reproducibleCode, takeaway。\n\n请基于以下论文信息，为研究生论文库生成中文结构化阅读笔记初稿。\n\n标题：${paper.title}\n作者：${paper.authors.join(", ")}\n分类：${paper.category}\n标签：${paper.tags.join(", ")}\n摘要：${paper.abstract}\n链接：${paper.url}`
    })
  });
  ...
  if (!response.ok) {
    return NextResponse.json(
      {
        message: `通义千问 API request failed: ${response.status}`
      },
      { status: 502 }
    );
  }
